...
レイアウトの「コンセプト型」カタログ。[ai-flow.md](../ai-flow.md) の concept-first hybrid において、Claude が今日の曲・concept と照合して採用 / 改変 / 自由作成を判断するための母体。

**使い方** (Claude 向け):
1. 今日の曲・concept を読む
2. 下記パターンのいずれかに合致するか判定する
3. 合致するなら base に書く、部分一致なら拡張・改変、合致しないなら自由に書く
4. 「ぴったり」を探す必要はない、参考にして自分のセンスで書いてもよい
...
**コンセプト**: 1 つの巨大な装飾領域 (= アートワーク) が画面の主役、記事は下部に track listing 風の細い行で並ぶ。
...
**コンセプト**: 雑誌の見開きページ。写真と本文が交互、引用が大きく、余白が呼吸する。記事は "読み物" として扱う。
...
**コンセプト**: ライブハウスのチラシ / ポスター。typography が暴れる、情報が積層、コントラスト強め、装飾満載。
...
**コンセプト**: コマンドラインインタフェース。monospace 主体、`$ prompt`、cursor 風点滅、緑 or アンバーの単色、限定色数。
...
**コンセプト**: 計算された崩し。中央寄せでなく、片側に寄ったレイアウト。余白の量がコンテンツ間で大きく違う、視線誘導が斜め。
...
**コンセプト**: 縦書き、和の感覚。`writing-mode: vertical-rl;` で全体が右から左に流れる。装飾は引き算、行頭の余白が呼吸する。
...
**コンセプト**: 1 つの記事 (例: 最新) を画面いっぱいに表示し、他の記事リストを最小化 (footer ノート的) する。今日の主役を 1 つ決める日。
...
**コンセプト**: スクラップブック / コラージュ。要素が transform で回転、重なり、複数のフォントが混在、layered。
